...
$cpf_hospede = $res[0]['cpf'];
$tel_hospede = $res[0]['telefone'];

echo '<small>1 - '.$nome_hospede.' - <b>Tel</b> '.$tel_hospede.' - <b>Doc:</b> '.$cpf_hospede.'</small>';
...
for($i=0; $i<$linhas; $i++){
	...
	$cpf_hospede = $res[$i]['cpf'];
	$id_hospede = $res[$i]['id'];
    $tel_hospede = $res[$i]['telefone'];

	echo '<br><small>'.$cont.' - '.$nome_hospede.' - <b>Tel</b> '.$tel_hospede.' - <b>Doc:</b> '.$cpf_hospede.'</small>';
